...
def validate_app_functionalities(app_page: "Page") -> None:
    ...
    while True:
        try:
            app_page.reload()
            sleep(5)
            app_label = app_page.frame_locator("iframe").locator("text='A demo of Masked Autoencoders research paper using Lightning App'")
    ...
    app_tabs = ["Poster", "Notebook", "Paper", "Model Demo"]
    for tab in app_tabs:
